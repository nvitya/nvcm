...
{
	TRACE("Internal Flash = %u k, sector = %u k, page = %u\r\n", bytesize >> 10, blocksize >> 10, pagesize);
}
	...
	if (INTFLASH_STATE_ERASE == state)
	{
		switch (phase)
		{
			...
			case 1: // prepare / continue
				blockchunk = blocksize;
				if (remaining < blockchunk)  blockchunk = remaining;  // important for the uint remaining
		...
		switch (phase)
		{
			...
			case 1: // prepare / continue
			{
				...
				if (match)
				{
					...
					srcaddr += (blockchunk >> 2); // adjust addresses here, which normally incremented during write
					dstaddr += (blockchunk >> 2);
					address += (blockchunk >> 2);
			...
			case 6: // start / continue write
				...
				if (blockremaining < chunksize)  chunksize = blockremaining;
